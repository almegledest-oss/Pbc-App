...
export const FocusTopBar: React.FC<FocusTopBarProps> = ({ onOpenNotifications }) => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-[#070D1B]/95 backdrop-blur-xl border-b border-[#D4AF37]/30 shadow-2xl transition-all">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 h-14 sm:h-16 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          {canGoBack ? (
            <button
              type="button"
              onClick={() => goBack()}
              className="flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-amber-500/20 to-amber-500/10 hover:from-amber-500/30 hover:to-amber-500/20 text-amber-300 hover:text-amber-200 rounded-xl border border-amber-500/40 shadow-sm transition active:scale-95 cursor-pointer shrink-0 font-bold text-xs sm:text-sm"
              title={isBn ? `পেছনে যান: ${prevTitle}` : `Go back to ${prevTitle}`}
            >
              <ArrowLeft className="w-4 h-4 stroke-[2.5]" />
              <span className="font-extrabold flex items-center gap-1">
                <span>{isBn ? 'পেছনে' : 'Back'}</span>
                <span className="hidden md:inline text-[11px] text-amber-400/80 font-normal">
                  ({prevTitle})
                </span>
              </span>
            </button>
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 bg-[#0B1528] hover:bg-[#112244] text-slate-300 hover:text-amber-300 rounded-xl border border-[#D4AF37]/30 transition active:scale-95 cursor-pointer shrink-0 text-xs font-bold"
          ...
          <div className="flex items-center gap-1.5 min-w-0">
            <button
              type="button"
              onClick={() => navigateWithHistory('dashboard')}
              className="hidden lg:flex items-center gap-1 text-xs text-slate-400 hover:text-amber-300 transition shrink-0"
            >
              <span>PBC</span>
            ...
            <div className="flex items-center gap-1.5 min-w-0">
              ...
              <h2 className="text-xs sm:text-sm md:text-base font-black text-white truncate tracking-wide">
                {currentTitle}
              </h2>
            </div>
            {isFocusMode && (
              <span className="hidden sm:inline-flex items-center gap-1 px-2 py-0.5 bg-amber-500/20 text-amber-300 border border-amber-500/40 rounded-md text-[10px] font-black tracking-wider uppercase shrink-0">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            type="button"
            onClick={() => navigateWithHistory('dashboard')}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-slate-300 hover:text-amber-300 bg-[#0B1528] hover:bg-[#112244] rounded-xl border border-[#D4AF37]/30 transition cursor-pointer active:scale-95 shadow-sm"
            title={isBn ? 'ড্যাশবোর্ডে ফিরে যান' : 'Go to Dashboard'}
          >
