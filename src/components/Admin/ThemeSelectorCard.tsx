...
export const ThemeSelectorCard: React.FC = () => {
  ...
  return (
    <div className={`p-5 sm:p-6 ${currentTheme.mode === 'light' ? 'bg-white border-amber-500/40 shadow-lg text-slate-900' : 'bg-[#070D1B] border-[#D4AF37]/30 text-white'} rounded-2xl border space-y-5`}>
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <div>
              <h4 className={`font-extrabold ${currentTheme.mode === 'light' ? 'text-slate-900' : 'text-white'} text-base flex items-center gap-2 flex-wrap`}>
                <span>App Color Themes & Aesthetics (অ্যাপের থিম পরিবর্তন)</span>
                {currentTheme.isDefault && (
                  <span className="px-2 py-0.5 bg-amber-500/20 text-amber-500 dark:text-amber-300 text-[10px] font-black rounded-md border border-amber-500/40">
                    DEFAULT ACTIVE
                  </span>
                )}
                {currentTheme.mode === 'light' ? (
                  <span className="px-2 py-0.5 bg-amber-400 text-slate-950 text-[10px] font-black rounded-md flex items-center gap-1 shadow-xs">
                    <Sun className="w-3 h-3" /> WHITE & GOLD ACTIVE
                  </span>
                ) : (
                  <span className="px-2 py-0.5 bg-slate-800 text-slate-300 text-[10px] font-black rounded-md flex items-center gap-1">
                    <Moon className="w-3 h-3 text-amber-400" /> ROYAL DARK ACTIVE
                  </span>
                )}
              </h4>
              <p className={`text-xs ${currentTheme.mode === 'light' ? 'text-slate-600' : 'text-slate-300'} mt-0.5`}>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {currentTheme.id !== DEFAULT_THEME_ID && (
            <button
              ...
              className={`px-3 py-1.5 ${currentTheme.mode === 'light' ? 'bg-slate-100 hover:bg-slate-200 text-slate-800 border-slate-300' : 'bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white border-slate-700'} rounded-xl text-xs font-bold border transition flex items-center gap-1.5 cursor-pointer`}
            >
          ...
          return (
            <button
              ...
              className={`p-4 rounded-2xl text-left transition-all relative overflow-hidden flex flex-col justify-between border cursor-pointer group ${
                isSelected
                  ? (currentTheme.mode === 'light' 
                      ? 'border-amber-500 shadow-xl ring-2 ring-amber-500/40 bg-amber-50/90' 
                      : 'border-amber-400 shadow-xl ring-2 ring-amber-400/40 bg-gradient-to-b from-[#0B1528] to-[#070D1B]')
                  : (currentTheme.mode === 'light' 
                      ? 'border-slate-200 hover:border-amber-400 bg-slate-50/80 hover:bg-white' 
            ...
            >
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    {isLight ? (
                      <span className="px-2 py-0.5 bg-amber-400/20 text-amber-600 dark:text-amber-300 text-[10px] font-bold rounded-md border border-amber-400/40 flex items-center gap-1">
                        <Sun className="w-2.5 h-2.5" /> Light
                      </span>
                    ) : (
                      <span className="px-2 py-0.5 bg-slate-800 text-slate-300 text-[10px] font-bold rounded-md border border-slate-700 flex items-center gap-1">
                ...
                <div>
                  <h5 className={`font-extrabold ${currentTheme.mode === 'light' ? 'text-slate-900 group-hover:text-amber-700' : 'text-white group-hover:text-amber-300'} text-sm transition`}>
                    {theme.name}
                  </h5>
                  <p className="text-[11px] font-medium text-amber-500 mt-0.5">
                    {theme.nameBn}
                  </p>
                  <p className={`text-[11px] ${currentTheme.mode === 'light' ? 'text-slate-600' : 'text-slate-400'} mt-1 line-clamp-2 leading-relaxed`}>
                    {theme.description}
                  </p>
                </div>
              </div>
              ...
              {/* Bottom tag */}
              <div className={`mt-3 pt-2.5 border-t ${currentTheme.mode === 'light' ? 'border-slate-200' : 'border-white/5'} flex items-center justify-between text-[10px] font-mono ${currentTheme.mode === 'light' ? 'text-slate-500' : 'text-slate-400'}`}>
                <span>
                  {theme.id === 'pearl-white-gold'
                    ? '✨ Ivory & Royal Gold'
                    : theme.isDefault
                    ? '⭐ Official Default'
                    : isLight
